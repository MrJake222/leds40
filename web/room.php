...
<body>
    ...
    <?php
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            if ($act == "addlist") {
                ...
                try {
                    $stmt = $db->prepare("INSERT INTO `lists` (`room_id`, `name`) VALUES (?, ?)");
                ...
                try {
                    ...
                    $stmt->execute([$list_id, $module_id]);
                ...
                try {
                    $stmt = $db->prepare("INSERT INTO `colors` (`room_id`, `name`, `red`, `grn`, `blu`) VALUES (?, ?, ?, ?, ?)");
                    $stmt->execute([$room_id, $name, $red, $grn, $blu]);
                }
                catch (mysqli_sql_exception $e) {
                    echo("[DB] insert failed: ".$db->error);
                }
                ...
                try {
                    ...
                    foreach($list_ids as $list_id) {
                        ...
                        $stmt = $db->prepare("INSERT INTO `listsel` (`room_id`, `list_id`) VALUES (?, ?);");
                        $stmt->execute([$room_id, $list_id]);
                    }
                }
                catch (mysqli_sql_exception $e) {
    ...
    <?php
        ...
        function get_all_selected_list_ids($room_id) {
            global $db;
            $stmt = $db->prepare("SELECT list_id FROM `listsel` WHERE `room_id` = ?");
            $stmt->execute([$room_id]);
        ...
        <div class="set-tables" id="verify-chk">
            <table>
                ...
                <?php
                    try {
                        // iterate over colors
                        $stmt = $db->prepare("SELECT color_id, name, red, grn, blu FROM `colors` WHERE `room_id` = ?");
                        $stmt->execute([$room_id]);
            ...
            <table>
                ...
                <?php
                        // get selected lists
                        $selected_list_ids = get_all_selected_list_ids($room_id);

                        // iterate over lists
                        $stmt = $db->prepare("SELECT list_id, name FROM `lists` WHERE `room_id` = ?");
                        $stmt->execute([$room_id]);
    ...
    <div class="lists indent">
        <?php
            ...
            function get_all_modules($room_id) {
                global $db;
                $stmt = $db->prepare("SELECT module_id, name FROM `modules` WHERE `room_id` = ?");
                $stmt->execute([$room_id]);
            ...
            try {
                ...
                $modules = get_all_modules($room_id);

                // iterate over lists
                $stmt = $db->prepare("SELECT list_id, name, red, grn, blu FROM `lists` WHERE `room_id` = ?");
                $stmt->execute([$room_id]);
